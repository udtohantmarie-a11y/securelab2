<!-- 🟢 SECURELAB MODERN EXECUTIVE SIDEBAR -->
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-brand-header px-4 py-3 border-bottom border-white border-opacity-10">
        <a href="{{ route('dashboard') }}" class="d-flex align-items-center gap-3 text-decoration-none">
            ...
            <div class="brand-text">
                ...
                <span class="brand-subtitle">IoT Access Control</span>
            </div>
        </a>
        ...
        <nav class="nav flex-column">
            ...
            <a href="{{ route('rooms.my') }}" 
               class="nav-link {{ Route::is('rooms.my') ? 'active' : '' }}">
                <i class="fas fa-door-open me-3 text-center" style="width: 20px;"></i>
            ...
            @if(in_array(Auth::user()?->role, ['Admin', 'Dean']))
                <a href="{{ route('audit.logs') }}" 
                   class="nav-link {{ Route::is('audit.logs') ? 'active' : '' }}">
                    <i class="fas fa-history me-3 text-center" style="width: 20px;"></i>
                    <span>Audit Logs</span>
                </a>
                <a href="{{ route('dashboard.alerts') }}" 
                   class="nav-link {{ Route::is('dashboard.alerts') ? 'active' : '' }}">
                    <i class="fas fa-exclamation-triangle me-3 text-center" style="width: 20px;"></i>
            ...
            @if(Auth::user()?->role == 'Admin')
                ...
                <a href="{{ route('users.assign') }}" 
                   class="nav-link {{ Route::is('users.assign') ? 'active' : '' }}">
                    <i class="fas fa-user-plus me-3 text-center" style="width: 20px;"></i>
                ...
                <a href="{{ route('users.pending') }}" 
                   class="nav-link {{ Route::is('users.pending') ? 'active' : '' }}">
                    <i class="fas fa-user-clock me-3 text-center" style="width: 20px;"></i>
                    <span>Pending Accounts</span>
                </a>
                
                <a href="{{ route('devices.manage') }}" 
                   class="nav-link {{ Route::is('devices.manage') ? 'active' : '' }}">
                    <i class="fas fa-microchip me-3 text-center" style="width: 20px;"></i>
                ...
                <a href="{{ route('hardware.wifi') }}" 
                   class="nav-link {{ Route::is('hardware.wifi') ? 'active' : '' }}">
                    <i class="fas fa-wifi me-3 text-center" style="width: 20px;"></i>
            ...
            @if(Auth::user()?->role == 'Dean')
                <div class="section-label mt-3">Department Reports</div>
                ...
                <a href="{{ route('dean.reports') }}" 
                   class="nav-link {{ Route::is('dean.reports') ? 'active' : '' }}">
                    <i class="fas fa-file-invoice me-3 text-center" style="width: 20px;"></i>
    ...
    <div class="sidebar-user-footer p-3 border-top border-white border-opacity-10">
        <div class="d-flex align-items-center justify-content-between p-2 rounded-3" style="background: rgba(255, 255, 255, 0.05);">
            <a href="{{ route('profile.index') }}" class="d-flex align-items-center gap-2 text-decoration-none text-truncate" style="max-width: 175px;">
                @if(Auth::user()?->profile_photo)
                    <img src="{{ asset('storage/' . Auth::user()->profile_photo) }}" class="rounded-circle border border-white border-opacity-25" width="34" height="34" style="object-fit: cover;">
                @else
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()?->full_name ?? 'User') }}&background=2563eb&color=fff" class="rounded-circle border border-white border-opacity-25" width="34" height="34">
                @endif
